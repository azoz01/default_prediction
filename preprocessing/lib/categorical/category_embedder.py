...
class CategoryEmbedder(TransformerMixin):
    ...
    def transform(
        ...
        """
        Encodes specified categorical columns using
        category embedding technique.
        ...
        """
        X = self._categorize_categorical_variables(X)
        tabular_pandas = self._get_tabular_pandas(X, valid_sample_frac=0)
        X_embedded = self._embed_features(self.nn_model, tabular_pandas)
